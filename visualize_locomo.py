...
def calculate_metrics_for_question(question_data):
    ...
    evidence = set(question_data['evidence'])
    if not evidence:
        return {}
    ...
    for retriever_name in retrievers:
        ...
        relevant_retrieved = len(retrieved_set & evidence)
        recall = relevant_retrieved / len(evidence) if evidence else 0.0
        precision = relevant_retrieved / len(retrieved) if retrieved else 0.0
        success = 1 if (retrieved_set & evidence) else 0
...
plt.tight_layout()
plt.savefig('locomo_overall_performance.png', dpi=300, bbox_inches='tight')
print("Saved: locomo_overall_performance.png")
plt.close()

# ========== FIGURE 2: Category-Wise Performance ==========
...
for question_data in data:
    ...
    for retriever in retrievers:
        if retriever in metrics:
            category_metrics[category][retriever].append(metrics[retriever]['recall@5'])

categories = sorted([c for c in category_metrics.keys() if isinstance(c, int)])
category_recall = {}
for category in categories:
    category_recall[category] = {}
    for retriever in retrievers:
        if retriever in category_metrics[category]:
            category_recall[category][retriever] = np.mean(category_metrics[category][retriever])

# Create heatmap
fig2, ax2 = plt.subplots(figsize=(10, 6))
heatmap_data = []
for retriever in retrievers:
    row = [category_recall[cat].get(retriever, 0) for cat in categories]
...
ax2.set_xticklabels([f'Category {c}' for c in categories], fontsize=11)
ax2.set_yticklabels(retrievers, fontsize=11)
ax2.set_xlabel('Question Category', fontsize=12, fontweight='bold')
ax2.set_ylabel('Retriever', fontsize=12, fontweight='bold')
ax2.set_title('Recall@5 by Retriever and Category', fontsize=14, fontweight='bold', pad=15)
...
plt.tight_layout()
plt.savefig('locomo_category_heatmap.png', dpi=300, bbox_inches='tight')
print("Saved: locomo_category_heatmap.png")
plt.close()

# ========== FIGURE 3: Category Comparison (Bar Chart) ==========
fig3, ax3 = plt.subplots(figsize=(14, 6))

# Show ALL retrievers per category
x = np.arange(len(categories))
width = 0.15  # Narrower bars to fit all 5 retrievers

for i, retriever in enumerate(retrievers):
    values = [category_recall[cat].get(retriever, 0) for cat in categories]
...
ax3.set_title('Recall@5 by Category (All Retrievers)', fontsize=14, fontweight='bold', pad=15)
ax3.set_xticks(x)
ax3.set_xticklabels([f'Category {c}' for c in categories], fontsize=11)
ax3.legend(fontsize=10, framealpha=0.9, ncol=5, loc='upper left')
ax3.grid(axis='y', alpha=0.3, linestyle='--')
ax3.set_ylim(0, 1.05)

plt.tight_layout()
plt.savefig('locomo_category_comparison.png', dpi=300, bbox_inches='tight')
print("Saved: locomo_category_comparison.png")
plt.close()
...
plt.tight_layout()
plt.savefig('locomo_distribution.png', dpi=300, bbox_inches='tight')
print("Saved: locomo_distribution.png")
plt.close()
...
print("="*70)
for category in categories:
    num_questions = len(category_metrics[category][retrievers[0]]) if retrievers[0] in category_metrics[category] else 0
    print(f"\nCategory {category} ({num_questions} questions):")
    print("-" * 70)
    sorted_retrievers = sorted(retrievers, 
                              key=lambda x: category_recall[category].get(x, 0), 
                              reverse=True)
    for retriever in sorted_retrievers:
        recall = category_recall[category].get(retriever, 0)
        print(f"  {retriever:<15} Recall@5: {recall:.4f}")
print("="*70)

print("\n✅ All visualizations saved successfully!")
print("   - locomo_overall_performance.png")
print("   - locomo_category_heatmap.png")
print("   - locomo_category_comparison.png")
print("   - locomo_distribution.png")
