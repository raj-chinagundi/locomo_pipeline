...
class EnsembleRetriever(BaseRetriever):
    ...
    def __init__(self, documents: List[Dict[str, Any]]):
        ...
        self.retrievers = {
            ...
            "faiss": FAISSRetriever(documents)
        }
    ...
    def retrieve(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        for name, retriever in self.retrievers.items():
            try:
                ...
                for rank, doc in enumerate(results):
                    doc_id = doc["id"]
                    # Use rank-based scoring (inverse rank)
                    score = 1.0 / (rank + 1)
                ...
                print(f"Warning: {name} retriever failed: {e}")
                continue
        ...
        return [all_results[doc_id] for doc_id in sorted_doc_ids[:top_k]]
...
def get_all_retrievers(documents: List[Dict[str, Any]]) -> Dict[str, BaseRetriever]:
    ...
    try:
        retrievers["ensemble"] = EnsembleRetriever(documents)
    except Exception as e:
        print(f"Failed to initialize EnsembleRetriever: {e}")
